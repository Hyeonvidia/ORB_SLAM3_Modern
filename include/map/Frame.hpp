...
public:
    ...
    // Copy constructor. R4c audit: the hand-written body is still required —
    // ~100 members with mixed semantics (cv::Mat clones vs. shared pointers,
    // grid array copy). Copy ASSIGNMENT stays implicitly generated
    // (memberwise, cv::Mat header-sharing), which upstream relies on in
    // Tracking (mCurrentFrame/mLastFrame hand-off) — do not unify the two.
    ...
    Frame(const cv::Mat &imGray, const cv::Mat &imDepth, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth, GeometricCamera* pCamera,Frame* pPrevF = nullptr, const IMU::Calib &ImuCalib = IMU::Calib());
...
public:
    ...
    // published by PreintegrateIMU (nullptr until then, and nullptr forever on its early
