...
# examples:
#   ./run.sh euroc_stereo MH01                 # GUI via VNC (Screen Sharing opens)
#   ./run.sh euroc_mono_inertial V102 --headless
#   ./run.sh kitti_stereo 07 --timeout 300     # bounded run (exit 124 = timeout)
#
# GUI is delivered over VNC (rendered in-container with llvmpipe/Xvfb,
# viewed with macOS Screen Sharing at vnc://127.0.0.1:5901 — opened
# automatically). Reason: current macOS XQuartz GLX gives container clients
# only indirect OpenGL 1.4, where Pangolin draws a blank window; --xquartz
# keeps that legacy path available in case a future update fixes it.
...
case "$GUI" in
  ...
  xquartz)
    ...
    echo "WARN: XQuartz GLX currently yields indirect GL 1.4 — Pangolin may show a blank Map Viewer." >&2 ;;
esac
[[ -n "$TIMEOUT" ]] && ENVS+=(-e RUN_TIMEOUT="$TIMEOUT")

# --- run ---------------------------------------------------------------------
START_EPOCH=$(date +%s)
docker compose run --rm "${RUNFLAGS[@]}" "${ENVS[@]}" dev docker/scripts/run_slam.sh "$MODE" "$SEQ"
RC=$?
