...
20. **[계측 사실/수용] 의미론적으로 무해한 삭제 커밋이 stereo-inertial ATE를
    ...
    - **P10 재계측 (2026-08-11)**: SI 8v8 **Δ=+4.0%, p=0.360 — 청정 2연속**.
      스레딩 현대화(케이던스 변경 포함) 후에도 재개 조건 미발동. 이 항목은
      이제 관찰 종료 후보 — P11 게이트에서 한 번 더 청정이면 CLOSED 처리.
    - **P11 재계측 + CLOSED (2026-08-11, 페이즈 마무리 게이트)**: SI 8v8
      **Δ=+2.1%, p=0.164 — 청정 3연속** (P9 −7.7% → P10 +4.0% → P11 +2.1%).
      FixLevel 전 플래그 OFF 상태의 풀 게이트 전 모드 PASS와 동시 판정
      (benchmark/gate_runs/20260811_103922/verdict.txt). 사전 선언된 종료
      기준 충족으로 **본 항목 CLOSED** — 상시 8v8 감시를 종료한다. 단
      레이아웃 민감성의 존재 실측은 유효하게 남으며, "XL 규모 diff는
      게이트로 귀속 불가"라는 분할 규율은 P12 이후에도 유지된다
      (docs/P12_PLAN.md §4). 재개 조건: 향후 게이트에서 SI가 p<0.05 악화를
      다시 보이면 신규 항목이 아니라 본 항목 재개로 다룬다.
