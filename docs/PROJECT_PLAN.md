...
| D8 | GTSAM 버전 (P12-D2, 2026-08-11 확정) | **4.3 대기**(Boost 제거판; 스테이블 부재 시 프리릴리스 태그 핀 허용, P12-G0 착수 시 릴리스 상태 재확인) | P12-S의 boost 탈피와 정렬 — 4.2.2-with-Boost는 S가 제거한 의존을 되끌고 옴 (docs/P12_PLAN.md §7) |
...
- **2단 게이트 구조** (2026-08-06 확정):
  ...
  - **풀 게이트 v2** (Phase 완료 시에만): 원본/신규를 **교차(interleaved paired)**
    실행해 순위합 검정(p<0.05 AND Δ중앙값>10%면 FAIL). 저장 기준선과의 절대 비교
    금지 — ATE는 머신 상태에 ±20% 흔들림이 P1에서 실측됨(docs/phase_reports/P1.md).
