...
namespace ORB_SLAM3 {
    ...
    Settings::Settings(const std::string &configFile, const Sensor sensor) :
    ...
    void Settings::readCamera2(cv::FileStorage &fSettings) {
        ...
        if(cameraType_ == Rectified){
            ...
            if(calibration1_){
                std::vector<float> vCal1 = {calibration1_->getParameter(0),calibration1_->getParameter(1),calibration1_->getParameter(2),calibration1_->getParameter(3)};
                calibration2_ = CameraFactory::create("PinHole", vCal1);
                originalCalib2_ = CameraFactory::create("PinHole", vCal1);
            }
    ...
    void Settings::readImageInfo(cv::FileStorage &fSettings) {
        ...
        if(found){
            ...
            if(!bNeedToRectify_){
                //Update calibration
                float scaleRowFactor = static_cast<float>(newImSize_.height) / static_cast<float>(originalImSize_.height);
        ...
        if(found){
            ...
            if(!bNeedToRectify_){
                //Update calibration
                float scaleColFactor = static_cast<float>(newImSize_.width) /static_cast<float>(originalImSize_.width);
